# 샘플 데이터베이스 설명 (Sample database description) — EPAS 18 SQL 기본 개념

> 원문: [EPAS v18 — Fundamentals › SQL fundamentals › Sample database description](https://www.enterprisedb.com/docs/epas/latest/fundamentals/sql_fundamentals/03_examples_used/)
> EPAS v18 공식 문서의 한글 **전문 번역**입니다. 설명은 번역하되, 스크립트(SQL)·테이블·데이터·경로·식별자는 원문 그대로 보존했습니다.

문서의 예제는 샘플 테이블 `dept`, `emp`, `jobhist`를 사용합니다. 이 테이블들은 **EDB Postgres Advanced Server 설치 시 생성·적재**됩니다.

## 사용 가능한 스크립트 (Available scripts)

언제든 다음 스크립트를 실행하여 샘플 데이터베이스의 테이블과 프로그램을 재생성할 수 있습니다.
...
또한 같은 디렉터리에는 Oracle 데이터베이스 호환 구문으로 데이터베이스 객체를 생성하는 스크립트가 있습니다. 해당 스크립트 파일은 `edb-sample.sql`이며, 이 스크립트는 다음을 수행합니다.

- 현재 연결된 데이터베이스에 샘플 테이블과 프로그램을 생성합니다.
- 테이블에 대한 모든 권한을 `PUBLIC` 그룹에 부여합니다.

테이블과 프로그램은 현재 사용자가 테이블·프로시저를 생성할 권한이 있는 search path의 첫 번째 스키마에 생성됩니다. search path는 다음 명령으로 확인할 수 있습니다.

```sql
SHOW SEARCH_PATH;
```

PSQL(PostgreSQL용 터미널 기반 인터페이스) 명령으로 search path를 수정할 수 있습니다.

## 샘플 데이터베이스 소개 (About the sample database)

샘플 데이터베이스는 한 조직의 직원을 표현합니다. 직원(employees), 부서(departments), 직원의 이력 기록(historical records) 세 가지 유형의 레코드를 포함합니다.

각 직원은 식별 번호, 이름, 입사일, 급여, 관리자를 가집니다. 일부 직원은 급여 외에 커미션(commission)을 받습니다. 모든 직원 관련 정보는 `emp` 테이블에 저장됩니다.

이 회사는 지역적으로 분산되어 있어 부서의 위치를 추적합니다. 각 직원은 하나의 부서에 배정됩니다. 각 부서는 고유한 부서 번호와 짧은 이름으로 식별되며, 하나의 위치와 연결됩니다. 모든 부서 관련 정보는 `dept` 테이블에 저장됩니다.

또한 회사는 직원이 맡았던 직무 정보를 추적합니다. 일부 직원은 오래 근무하며 여러 직위를 거치고, 급여 인상, 부서 이동 등을 경험했습니다. 직원 상태에 변경이 생기면 회사는 이전 직위의 종료일을 기록하고, 시작일·새 직무명·부서·급여·변경 사유를 담은 새 직무 레코드를 추가합니다. 모든 직원 이력은 `jobhist` 테이블에 유지됩니다.

다음은 `pg-sample.sql` 스크립트입니다.
