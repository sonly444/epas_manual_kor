# 종합 예제 (Comprehensive example) — EPAS 18 내장 패키지 DBMS_PIPE
> 원문: [EPAS v18 — Reference › Oracle compatibility › Built-in packages › DBMS_PIPE › 종합 예제 (Comprehensive example)](https://www.enterprisedb.com/docs/epas/latest/reference/oracle_compatibility_reference/epas_compat_bip_guide/03_built-in_packages/10_dbms_pipe/11_comprehensive_example/)
> EPAS v18 공식 문서의 한글 **전문 번역**입니다. 패키지·프로시저·함수 이름은 원문 그대로 보존했습니다.

이 예제는 파이프를 "메일박스(mailbox)"로 사용합니다. `mailbox`라는 이름의 패키지 안에 세 개의 프로시저가 들어 있습니다. 이 프로시저들은 다음을 수행합니다.

1. 메일박스를 생성합니다.
2. 메일박스에 여러 항목으로 구성된 메시지를 추가합니다(최대 세 개 항목).
3. 메일박스의 전체 내용을 표시합니다.
...
다음은 `mailbox`의 프로시저들을 실행하는 모습입니다. 첫 번째 프로시저는 `UNIQUE_SESSION_NAME` 함수로 생성한 이름을 사용하여 공용 파이프를 생성합니다.
...
이 메일박스 이름을 사용하면, `mailbox` 패키지와 `DBMS_PIPE` 패키지에 접근할 수 있는 동일 데이터베이스의 모든 사용자가 메시지를 추가할 수 있습니다.
