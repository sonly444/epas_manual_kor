# FETCH_ROWS — EPAS 18 내장 패키지 DBMS_SQL
> 원문: [EPAS v18 — Reference › Oracle compatibility › Built-in packages › DBMS_SQL › FETCH_ROWS](https://www.enterprisedb.com/docs/epas/latest/reference/oracle_compatibility_reference/epas_compat_bip_guide/03_built-in_packages/17_dbms_sql/14_fetch_rows/)
> EPAS v18 공식 문서의 한글 **전문 번역**입니다. 패키지·프로시저·함수 이름은 원문 그대로 보존했습니다.

`FETCH_ROWS` 함수는 커서로부터 행을 검색합니다.
...
행을 성공적으로 가져오면 `1`을, 더 이상 가져올 행이 없으면 `0`을 반환합니다.

## 예제 (Example)

다음 예제는 `emp` 테이블에서 행을 가져와 결과를 표시합니다.
